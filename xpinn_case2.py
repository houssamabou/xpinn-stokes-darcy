"""
XPINN Stokes-Darcy - CAS 2
Domaines rectangulaires, solution discontinue à l'interface (y = 0).
"""
...
class ExactSolutionCase2:
    """Solution discontinue à l'interface, domaines rectangulaires"""
    @staticmethod
    def u_s(x, y):
    ...
    @staticmethod
    def p_s(x, y):
        return 0.5 * torch.sin(math.pi * x) * torch.cos(math.pi * y)
    @staticmethod
    def u_d(x, y):
        return 0.5 * torch.sin(2 * math.pi * x) * torch.cos(2 * math.pi * y)
    @staticmethod
    def v_d(x, y):
        return -0.5 * torch.cos(2 * math.pi * x) * torch.sin(2 * math.pi * y)
    @staticmethod
    def p_d(x, y):
        return 0.5 * torch.sin(math.pi * x) * torch.cos(math.pi * y)
...
def build_training_data(cfg):
    ...
    ub_s = Exact.u_s(xs_b, ys_b).detach()
    vb_s = Exact.v_s(xs_b, ys_b).detach()
    ub_d = Exact.u_d(xd_b, yd_b).detach()
    vb_d = Exact.v_d(xd_b, yd_b).detach()

    u_e = Exact.u_s(xs_f, ys_f); v_e = Exact.v_s(xs_f, ys_f); p_e = Exact.p_s(xs_f, ys_f)
    ...
    u_e = Exact.u_d(xd_f, yd_f); v_e = Exact.v_d(xd_f, yd_f); p_e = Exact.p_d(xd_f, yd_f)
    p_x = grad(p_e, xd_f); p_y = grad(p_e, yd_f)
    f_darcy_1 = (cfg.nu * (1. / cfg.K) * u_e + p_x).detach()
    f_darcy_2 = (cfg.nu * (1. / cfg.K) * v_e + p_y).detach()

    u_g = Exact.u_s(x_g, y_gs); v_g = Exact.v_s(x_g, y_gs)
    p_sg = Exact.p_s(x_g, y_gs); p_dg = Exact.p_d(x_g, y_gd)
...
class XPINNSolver:
    ...
    def compute_metrics(self):
        ...
        p_s_ex = center(self.Exact.p_s(xs, ys))
        p_d_ex = center(self.Exact.p_d(xd, yd))
        u_s_ex = self.Exact.u_s(xs, ys); v_s_ex = self.Exact.v_s(xs, ys)
        u_d_ex = self.Exact.u_d(xd, yd); v_d_ex = self.Exact.v_d(xd, yd)
...
def plot_convergence(history, cfg):
    ...
    plt.tight_layout()
    plt.savefig(f"{cfg.output_dir}/convergence_case2.png", dpi=cfg.dpi, bbox_inches='tight')
    plt.close()
...
def plot_global_fields(solver, cfg):
    ...
    u_ex = torch.zeros_like(xg); v_ex = torch.zeros_like(xg); p_ex = torch.zeros_like(xg)
    if mask_s.any():
        u_ex[mask_s] = solver.Exact.u_s(xg[mask_s], yg[mask_s])
        v_ex[mask_s] = solver.Exact.v_s(xg[mask_s], yg[mask_s])
        p_ex[mask_s] = solver.Exact.p_s(xg[mask_s], yg[mask_s])
    if mask_d.any():
        u_ex[mask_d] = solver.Exact.u_d(xg[mask_d], yg[mask_d])
        v_ex[mask_d] = solver.Exact.v_d(xg[mask_d], yg[mask_d])
        p_ex[mask_d] = solver.Exact.p_d(xg[mask_d], yg[mask_d])
    ...
    plt.tight_layout()
    plt.savefig(f"{cfg.output_dir}/global_fields_case2.png", dpi=cfg.dpi, bbox_inches='tight')
    plt.close()
...
def plot_region_fields(solver, cfg, region='stokes'):
    ...
    u, v, p = model(xg, yg)
    if region == 'stokes':
        u_ex = solver.Exact.u_s(xg, yg)
        v_ex = solver.Exact.v_s(xg, yg)
        p_ex = solver.Exact.p_s(xg, yg)
    else:
        u_ex = solver.Exact.u_d(xg, yg)
        v_ex = solver.Exact.v_d(xg, yg)
        p_ex = solver.Exact.p_d(xg, yg)
    ...
    plt.tight_layout()
    plt.savefig(f"{cfg.output_dir}/fields_{region}_case2.png", dpi=cfg.dpi, bbox_inches='tight')
    plt.close()
...
def run():
    ...
    with open(f"{cfg.output_dir}/final_metrics_case2.txt", "w") as f:
