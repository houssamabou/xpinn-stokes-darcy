"""
XPINN Stokes-Darcy - CAS 3
Interface courbe y = 0.0625 sin(4 pi x).
"""
...
# ============================================================
# SOLUTION EXACTE - CAS 3
# ============================================================

def get_curve(x):
    return 0.0625 * torch.sin(4 * math.pi * x)

class ExactSolutionCase3:
    """Interface courbe"""
    @staticmethod
    def u(x, y):
        return (2.0 / math.pi) * torch.sin(math.pi * y) * torch.cos(math.pi * y) * torch.cos(x)
    @staticmethod
    def v(x, y):
        return (-2.0 + (1.0 / (math.pi**2)) * (torch.sin(math.pi * y)**2)) * torch.sin(x)
    @staticmethod
    def p(x, y):
        return (torch.exp(y) - torch.exp(-y)) * torch.sin(x)
...
def build_training_data(cfg):
    dtype_ = cfg.dtype
    Exact = ExactSolutionCase3

    # Points intérieurs séparés par la courbe (sur-échantillonnage puis filtrage par masque)
    x_all, y_all = lhs_2d(cfg.Nfs * 3, 0., 1., -1., 1., device, dtype_)
    curve_y = get_curve(x_all)
    mask_s = (y_all >= curve_y).squeeze()
    mask_d = (y_all < curve_y).squeeze()

    xs_f = x_all[mask_s][:cfg.Nfs].reshape(-1, 1).requires_grad_(True)
    ys_f = y_all[mask_s][:cfg.Nfs].reshape(-1, 1).requires_grad_(True)
    xd_f = x_all[mask_d][:cfg.Nfd].reshape(-1, 1).requires_grad_(True)
    yd_f = y_all[mask_d][:cfg.Nfd].reshape(-1, 1).requires_grad_(True)

    assert xs_f.shape[0] == cfg.Nfs, "Pas assez de points Stokes échantillonnés : augmenter le facteur de sur-échantillonnage"
    assert xd_f.shape[0] == cfg.Nfd, "Pas assez de points Darcy échantillonnés : augmenter le facteur de sur-échantillonnage"

    # Interface courbe
    x_g_raw = lhs_1d(cfg.NuGamma, 0., 1., device, dtype_).reshape(-1, 1)
    y_g_raw = get_curve(x_g_raw)
    x_g = x_g_raw.clone().detach().requires_grad_(True)
    y_gs = y_g_raw.clone().detach().requires_grad_(True)
    y_gd = y_g_raw.clone().detach().requires_grad_(True)
...
def plot_convergence(history, cfg):
    ...
    plt.tight_layout()
    plt.savefig(f"{cfg.output_dir}/convergence_case3.png", dpi=cfg.dpi, bbox_inches='tight')
    plt.close()
...
def plot_global_fields(solver, cfg):
    ...
    curve_yg = get_curve(xg)
    mask_s = (yg >= curve_yg).squeeze()
    mask_d = (yg < curve_yg).squeeze()
    ...
    y_line = 0.0625 * np.sin(4 * np.pi * x_line)
    ...
    plt.tight_layout()
    plt.savefig(f"{cfg.output_dir}/global_fields_case3.png", dpi=cfg.dpi, bbox_inches='tight')
    plt.close()
...
def plot_region_fields(solver, cfg, region='stokes'):
    ...
    plt.tight_layout()
    plt.savefig(f"{cfg.output_dir}/fields_{region}_case3.png", dpi=cfg.dpi, bbox_inches='tight')
    plt.close()
...
def run():
    ...
    with open(f"{cfg.output_dir}/final_metrics_case3.txt", "w") as f:
